...
df = pd.read_csv('data/train.csv', nrows=1000, sep='###__###', on_bad_lines='skip', engine='python', header=None, names=col_names)
...
def analyze_query_by_label(label_col, label_name_map):
    ...
    for label, label_name in label_name_map.items():
        ...
        words = [w for w in queries.split('|') if w.strip()]
        counter = Counter(words)
        print(f'\n{label_name}（{label}）最常见的前10个查询词:')
        print(counter.most_common(10))
        if len(counter) == 0:
            print(f'{label_name}（{label}）无有效查询词，跳过词云生成。')
            continue
        # 生成词云
        try:
...
def query_stats_by_label(label_col, label_name_map):
    print(f'\n===== 按{label_col}分组的查询词数量/多样性统计 =====')
    for label, label_name in label_name_map.items():
        group = df[df[label_col] == label]
        query_lists = group['Query List'].dropna().tolist()
        total_queries = [q for line in query_lists for q in line.split('|') if q.strip()]
        avg_query_count = sum(len(line.split('|')) for line in query_lists) / len(query_lists) if query_lists else 0
        unique_query_count = len(set(total_queries))
        print(f'{label_name}（{label}）：平均查询词数 {avg_query_count:.2f}，唯一查询词数 {unique_query_count}')
